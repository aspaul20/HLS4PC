...
int main() {
    ...
    auto input_data = read_hex_file<INPUT_PRECISION>("q_3216_input.txt");
    for (auto &u : input_data) {
        ap_uint<SIMD_WIDTH*INPUT_PRECISION> tmp = u;
        in_stream.write(tmp);
    }

    auto weights_data = read_hex_file<WEIGHTS_PRECISION>("q_3216_weights.txt");
    if (weights_data.size() < PE_COUNT*SIMD_WIDTH*WEIGHT_MEM_SIZE) {
    	cout << weights_data.size() << " != " << PE_COUNT*SIMD_WIDTH*WEIGHT_MEM_SIZE << endl;
    ...
    auto biases_data = read_hex_file<BIAS_PRECISION>("q_3216_biases.txt");
    ...
    auto gt_output  = read_hex_file<OUTPUT_PRECISION>("q_3216_output.txt");
